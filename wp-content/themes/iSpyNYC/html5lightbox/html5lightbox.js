    ...
    (function ($) {
        $.fn.html5lightbox = function (options) {
                ...
                if (inst.options.showsocial) {
                    ...
                    $(".html5-social-btn", inst.$lightbox).click(function () {
                        ...
                        if (inst.currentElem[ELEM_TYPE] == 0)shareMedia = inst.absoluteUrl(inst.currentElem[ELEM_HREF]); else if (inst.currentElem[ELEM_TYPE] == 3)shareMedia = "https://img.youtube.com/vi/" + inst.getYoutubeId(inst.currentElem[ELEM_HREF]) + "/0.jpg"; else {
                            ...
                            if (lightboxLink.length > 0)if (lightboxLink.data("shareimage") && lightboxLink.data("shareimage").length > 0)shareMedia = inst.absoluteUrl(lightboxLink.data("shareimage")); else if (lightboxLink.data("thumbnail") &&
                                lightboxLink.data("thumbnail").length > 0)shareMedia = inst.absoluteUrl(lightboxLink.data("thumbnail")); else {
            ...
            inst.prevArrowClicked = function () {
                if (inst.options.prevElem >= inst.options.curElem)if (inst.options.onfirstarrowclicked && window[inst.options.onfirstarrowclicked] &&
            ...
            inst.calcNextPrevElem = function () {
                ...
                if (curGroup != undefined && curGroup != null) {
                    ...
                    if (inst.options.nextElem >= 0) {
                        for (j = inst.options.curElem - 1; j >= 0; j--)if (inst.matchGroup(curGroup,
            ...
            inst.showMP3 = function (elem) {
                inst.slideTimeout.stop();
                var dataW = DEFAULT_WIDTH;
                var dataH = DEFAULT_HEIGHT;
                var videoSrc = elem[ELEM_HREF];

                inst.resizeLightbox(dataW, dataH, true, function () {
                    inst.$image.html("<div id='html5lightbox-video' style='display:block;width:100%;height:100%;overflow:hidden;background-color:" + inst.options.videobgcolor + ";'><video src='" + videoSrc + "' style='width: 80%' controls></div>").show();
                })
            };
